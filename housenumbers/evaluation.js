...
function exportofficialgeocoordinates(request, response) {
	...
	response.simpleQuery({name: "hnr_koordinaten", text: query_string, values: [params.job_id]}, 
		...
		if(hausnummern.length > 0) {
			...
			response.render("evaluation_missingparameters.html", 
				{	fortsetzurl: "/housenumbers/showevaluation?job_ids=" + params.job_id,
					fehlermeldung: request.i18n.__("no official geocoordinates available")});
		}
...
//---------------------------------------------------------------------------------------------
function checkosmgeocoordinates(request, response) {
	console.time("checkosmgeocoordinates");
	...
	for(var parami in params) {
		console.log("in evaluation/checkosmgeocoordinates: params["+parami+"]===" + params[parami] + "===");
	}
...
*/
	...
	response.simpleQuery({name: "hnr_koordinaten", text: query_string, values: [params.job_id]}, 
		...
		for(var rowi = 0; rowi < result.rows.length; rowi++) {
			hausnummern.push(result.rows[rowi]);
		}
	
		console.log("Count matched housenumbers: " + hausnummern.length);

		if(hausnummern.length > 0) {
			response.render("checkosmgeocoordinates.html", {params: params, hausnummern: hausnummern});
		} else {
			response.render("evaluation_missingparameters.html", 
				{	fortsetzurl: "/housenumbers/showevaluation?job_ids=" + params.job_id,
					fehlermeldung: request.i18n.__("no official geocoordinates available")});
		...
		console.timeEnd("checkosmgeocoordinates");
	});
}
//---------------------------------------------------------------------------------------------
//checkosmgeocoordinates
//=============================================================================================
...
function calculateCoordinateForAMissingHousenumber(strasse, strasseLonLat, hausnummerFehlend, osmHnrobjekte, osmHnrobjekteAnzahl) {
		// parameter hausnummerFehlend is still missing in osm and its potential geocoordindates will be evaluated
		// with the available other housenumbers in parameter osmHnrobjekte
	var LONGUNSET = 4711;
...
exports.checkosmgeocoordinates = checkosmgeocoordinates;
exports.show = show;
